            ...
            if ((btcProfitable(prlbtc, prleth, ethbtc) + .0045) < 1.0 && btc > .001) {
              ...
              if (amountprl > 1.0) {
                ...
                kc.createOrder({pair: 'PRL-BTC', amount: amountprl, price: prlbtc["SELL"][0][0], type: 'BUY'}).then(function(res){
                  ...
                  console.log(amountprl);
                  setTimeout(function() {
                    kc.createOrder({pair: 'PRL-ETH', amount: amountprl, price: .00001, type: 'SELL'}).then(function(res) {
              ...
              if (amountprl > 1.0) {
                ...
                kc.createOrder({pair: 'PRL-ETH', amount: amountprl, price: prleth["SELL"][0][0], type: 'BUY'}).then(function(res){
                  ...
                  console.log(amountprl);
                  setTimeout(function() {
                    kc.createOrder({pair: 'PRL-BTC', amount: amountprl, price: .00001, type: 'SELL'}).then(function(res) {
                      btc += (amountprl * prlbtc["BUY"][0][0]) * .9989;
                      inData = false;
                      console.log("TRADED");
                      console.log("BTC: " + btc);
                      console.log("ETH: " + eth);
